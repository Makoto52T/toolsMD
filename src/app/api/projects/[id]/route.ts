...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [projects] = await pool.query('SELECT * FROM projects WHERE id = ? AND user_id = ?', [params.id, userId]);
  if (!(projects as any[]).length) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const project = (projects as any[])[0];
  const [nodes] = await pool.query('SELECT * FROM nodes WHERE project_id = ? ORDER BY created_at ASC', [params.id]);
  const [funcs] = await pool.query(
    'SELECT f.* FROM functions f JOIN nodes n ON f.node_id = n.id WHERE n.project_id = ? ORDER BY f.sort_order ASC',
    [params.id]
  );
  const [edges] = await pool.query('SELECT * FROM edges WHERE project_id = ? ORDER BY created_at ASC', [params.id]);
